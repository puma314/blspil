...
class F1 {
  ...
  is_eq(a, b) {
    ...
    const res = this.I.sub(a, b);
    return this.I.is_zero(res);
  }
...
class F2 {
  ...
  is_eq(a, b) {
    // returns 1 if equal, 0 otherwise
    const res = this.F.is_eq(a[0], b[0]) * this.F.is_eq(a[1], b[1]);
    return 1n - this.F.I.is_zero(res);
  }
...
class F3 {
  ...
  is_eq(a, b) {
    // returns 1 if equal, 0 otherwise
    const res =
      this.F.is_eq(a[0], b[0]) *
      this.F.is_eq(a[1], b[1]) *
      this.F.is_eq(a[2], b[2]);
    return 1n - this.F.I.is_zero(res);
  }
