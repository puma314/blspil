...
const argv = require("yargs")
  .version(version)
  .usage(
    "node main_exec_fibonacci.js -o <fibonacci.commit.bin> -i <input.json>"
  )
  .alias("o", "output")
  .alias("i", "input").argv;
...
async function run() {
  const outputFile =
    typeof argv.output === "string" ? argv.output.trim() : "fibonacci.commit";
  const inputFile =
    typeof argv.input === "string" ? argv.input.trim() : "input.json";
